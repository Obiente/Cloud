...
func HandlePushDNSRecord(w http.ResponseWriter, r *http.Request) {
	...
	// Validate API key
	valid, err := database.ValidateDNSDelegationAPIKey(apiKey)
	if err != nil {
	...
	// Get source API URL from request (for tracking)
	...
	// Upsert the delegated DNS record
	if err := database.UpsertDelegatedDNSRecord(domain, recordType, string(recordsJSON), sourceAPI, ttl); err != nil {
...
func HandlePushDNSRecords(w http.ResponseWriter, r *http.Request) {
	...
	// Get source API URL from request
	sourceAPI := r.Header.Get("X-Source-API")
	if sourceAPI == "" {

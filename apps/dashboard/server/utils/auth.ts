...
export async function exchangeCodeForTokens(
  ...
  const response = await $fetch<ZitadelTokenResponse>(`${config.public.oidcBase}/oauth/v2/token`, {
    method: 'POST',
...
export async function getUserData(event: H3Event, session: UserSession): Promise<void> {
  const config = useRuntimeConfig();

  const response = await $fetch<User>(`${config.public.oidcBase}/oidc/v1/userinfo`, {
    headers: {

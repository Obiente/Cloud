  ...
  <div class="p-6">
    <OuiStack gap="lg">
      ...
      <OuiCard v-if="isLoading" variant="outline">
        <OuiCardBody>
          <OuiFlex align="center" gap="md">
            ...
            <OuiText size="sm" color="secondary">Loading connected accounts...</OuiText>
          </OuiFlex>
        </OuiCardBody>
      </OuiCard>
      ...
      <OuiCard v-else-if="integrations.length > 0" variant="outline">
        <OuiCardBody>
          <OuiStack gap="md">
            <OuiText as="h3" size="lg" weight="semibold">Active Connections</OuiText>
        ...
        <OuiCardBody>
          <OuiStack gap="md" align="center" class="py-8">
            ...
            <OuiStack gap="xs" align="center">
              <OuiText size="lg" weight="semibold">No connected accounts</OuiText>
              <OuiText size="sm" color="secondary" class="text-center max-w-md">
                Connect your GitHub account or an organization account to enable repository imports and deployments
              </OuiText>
            </OuiStack>
          </OuiStack>
        </OuiCardBody>
      ...
      <OuiCard variant="outline">
        <OuiCardBody>
          <OuiStack gap="lg">
            <OuiStack gap="xs">
              <OuiText as="h3" size="lg" weight="semibold">Connect New Account</OuiText>
              <OuiText size="sm" color="secondary">
                Connect a GitHub account to enable repository imports and deployments
              </OuiText>
            </OuiStack>
      ...
      <OuiCard variant="outline" class="opacity-50">
        <OuiCardBody>
          <OuiStack gap="md">
            ...
            <OuiText size="sm" color="secondary">
              Coming soon - Connect your GitLab account for repository integration
            </OuiText>
          </OuiStack>
        </OuiCardBody>
      </OuiCard>
  ...
  if (provider === "github") {
    ...
    if (success === "true" && username) {
      ...
      if (errorMsg === "missing_code") {
        error.value = "Authorization code missing. Please try connecting again.";
      } else if (errorMsg === "configuration_error") {
        error.value = "GitHub integration is not properly configured. Please contact your administrator.";
...
const connectGitHub = () => {
  ...
  if (!githubClientId || githubClientId === "") {
    error.value = "GitHub Client ID not configured. Please set NUXT_PUBLIC_GITHUB_CLIENT_ID in your .env file.";
  ...
  const stateData = {
    ...
    orgId: connectionType.value === "organization" && selectedOrgId.value ? selectedOrgId.value : undefined,
  };
  ...
  const authUrl = `https://github.com/login/oauth/authorize?client_id=${githubClientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(scope)}&state=${encodeURIComponent(state)}`;
...
const disconnectIntegration = async (integration: typeof integrations.value[0]) => {
  const accountName = integration.isUser 
    ? `your personal account (${integration.username})`
    : `organization ${integration.organizationName || integration.organizationId} (${integration.username})`;
  ...
  try {
    const { useConnectClient } = await import("~/lib/connect-client");
    const { AuthService, DisconnectGitHubRequestSchema, DisconnectOrganizationGitHubRequestSchema } = await import("@obiente/proto");
    const { create } = await import("@bufbuild/protobuf");
    ...
    error.value = err.message || "Failed to disconnect GitHub account. Please try again.";
  } finally {

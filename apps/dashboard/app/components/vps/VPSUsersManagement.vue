  ...
  <OuiStack gap="lg">
              ...
              <OuiFlex justify="between" align="start">
                <OuiStack gap="xs">
                  <OuiFlex align="center" gap="sm">
                    ...
                    <OuiBadge v-if="user.sudo" variant="primary" size="xs"
                      >Sudo</OuiBadge
                    >
                    <OuiBadge
                      v-if="user.sudoNopasswd"
                      variant="success"
                      size="xs"
                      >Sudo (NOPASSWD)</OuiBadge
                    >
                  </OuiFlex>
                  <OuiText
                ...
                <OuiStack gap="xs">
                  ...
                  >
                    <OuiFlex justify="between" align="center" gap="sm">
                      <OuiText class="truncate"
                        >{{ key.substring(0, 60) }}...</OuiText
                      >
                      <OuiButton
                        variant="ghost"
                        size="xs"
    ...
    >
      <OuiStack gap="md">
        ...
        />
        <OuiCheckbox v-model="newUser.sudo" label="Grant sudo access" />
        <OuiCheckbox
        ...
        <OuiStack gap="xs">
          ...
          <OuiStack v-else gap="xs" class="max-h-48 overflow-y-auto">
            <OuiCheckbox
      ...
      <template #footer>
        <OuiFlex justify="end" gap="sm">
          <OuiButton variant="ghost" @click="closeCreateUserDialog"
            >Cancel</OuiButton
          >
          <OuiButton
            variant="solid"
    ...
    >
      ...
      <template #footer>
        <OuiFlex justify="end" gap="sm">
          <OuiButton variant="ghost" @click="manageSSHKeysDialogOpen = false"
            >Cancel</OuiButton
          >
          <OuiButton
            variant="solid"
            @click="saveUserSSHKeys"
    ...
    <OuiDialog
      ...
      :title="`${
        selectedUser?.name === 'root'
          ? 'Reset'
          : selectedUser?.hasPassword
          ? 'Reset'
          : 'Set'
      } Password for ${selectedUser?.name || 'User'}`"
      size="md"
    >
      <OuiStack gap="md" v-if="selectedUser">
        <OuiInput
      ...
      <template #footer>
        <OuiFlex justify="end" gap="sm">
          <OuiButton variant="ghost" @click="resetPasswordDialogOpen = false"
            >Cancel</OuiButton
          >
          <OuiButton
            variant="solid"
...
<script setup lang="ts">
  ...
  const createUser = async () => {
    ...
    try {
      ...
      toast.success(
        "User created",
        res.message || "The user will be created on the next reboot."
      );
  ...
  const deleteUser = async (username: string) => {
    ...
    try {
      ...
      toast.success(
        "User deleted",
        res.message || "The user will be removed on the next reboot."
      );
      await loadUsers();
    } catch (err: unknown) {
      toast.error(
  ...
  const saveUserSSHKeys = async () => {
    ...
    try {
      ...
      toast.success(
        "SSH keys updated",
        res.message || "Changes will be applied on the next reboot."
      );
  ...
  const openResetRootPasswordDialog = () => {
    ...
    toast.info(
      "Root password reset",
      "Please use the SSH Settings tab to reset the root password."
    );
  };
  ...
  const saveUserPassword = async () => {
    ...
    try {
      ...
      toast.success(
        "Password updated",
        res.message || "The password will be applied on the next reboot."
      );
  ...
  const removeSSHKeyFromUser = async (username: string, keyIndex: number) => {
    ...
    const updatedKeys = user.sshAuthorizedKeys.filter(
      (_, idx) => idx !== keyIndex
    );

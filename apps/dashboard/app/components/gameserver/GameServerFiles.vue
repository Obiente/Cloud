...
<script setup lang="ts">
  ...
  async function handleCreate(type: "file" | "directory" | "symlink") {
    ...
    let entryType = GameServerEntryType.FILE;
    if (type === "directory") entryType = GameServerEntryType.DIRECTORY;
    if (type === "symlink") entryType = GameServerEntryType.SYMLINK;

    const payload: Partial<CreateGameServerEntryRequest> = {
      parentPath: parent,
      name,
      type: entryType,
  ...
  onMounted(async () => {
    ...
    // Always try to fetch volumes when mounted
    await fetchVolumes();
    await refreshRoot();

          ...
          <OuiCardBody>
            <OuiStack gap="md">
              <OuiFlex justify="between" align="start">
                <OuiText size="sm" weight="semibold">Rule {{ index + 1 }}</OuiText>
                <OuiButton
                  variant="ghost"
                  size="sm"
                  color="danger"
              ...
              <OuiGrid cols="1" :cols-md="2" gap="md">
                <OuiInput
                  ...
                  label="Target Port"
                  placeholder="80"
                  @update:model-value="(val) => { rule.targetPort = parseInt(val) || 80; rule.targetPortStr = val; markDirty(); }"
                />
                <OuiInput
...
const saveRules = async () => {
  ...
  try {
    ...
    await showAlert({
      title: "Success",
      message: "Routing rules saved successfully. Note: Changes will take effect on the next deployment.",
    });
  } catch (error: any) {

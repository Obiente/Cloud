<template>
  <Toast.Root
    :class="[
      'bg-surface-overlay border rounded-2xl shadow-lg p-4 min-w-[320px] max-w-[560px] sm:w-auto',
      'translate-(--x) translate-y-(--y) scale-(--scale)',
      'z-(--z-index) h-(--height) opacity-(--opacity)',
      'will-change-[translate,opacity,scale]',
      'transition-[translate,scale,opacity,height,box-shadow] duration-400',
      'ease-[cubic-bezier(0.21,1.02,0.73,1)]',
      'data-[state=closed]:transition-[translate,scale,opacity] data-[state=closed]:duration-400',
  ...
  >
    <OuiFlex align="start" justify="between" gap="md" class="w-full">
      ...
      <Toast.CloseTrigger
        class="shrink-0 p-1 rounded hover:bg-surface-muted text-text-secondary hover:text-foreground transition-colors"
      >

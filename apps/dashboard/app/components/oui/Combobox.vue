<template>
  ...
  >
    <Combobox.Label v-if="label" class="block text-sm font-medium text-primary mb-1">
      {{ label }}
    </Combobox.Label>
    ...
    <Combobox.Control class="relative">
      ...
      >
        <XMarkIcon class="h-4 w-4 text-secondary hover:text-primary cursor-pointer" />
      </Combobox.ClearTrigger>
    </Combobox.Control>
    ...
    <Teleport to="body">
      <Combobox.Positioner>
        <Combobox.Content
          class="
            card-base max-h-60 overflow-auto z-50
        ...
        >
          ...
          <div v-if="collection.items.length === 0" class="px-3 py-2 text-sm text-text-secondary">
            No results found
          </div>
        </Combobox.Content>
      </Combobox.Positioner>

...
⚠️ **Important:** For template-based VMs (linked clones), the storage pool is inherited from the template and `PROXMOX_STORAGE_POOL` is ignored for the cloned disk. This setting only applies to VMs created without templates, new disks created when templates have no disk, or additional disks. To use a different storage pool for cloned VMs, create the template on the desired storage pool.

See the [VPS Configuration Guide](../guides/vps-configuration.md#storage-configuration) for details on checking available storage pools and template storage considerations.

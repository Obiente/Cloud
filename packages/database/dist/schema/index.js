import { pgTable, uuid, varchar, text, timestamp, jsonb, integer, decimal, pgEnum } from 'drizzle-orm/pg-core';
import { relations } from 'drizzle-orm';
// Enums
export const userRoleEnum = pgEnum('user_role', ['OWNER', 'ADMIN', 'DEVELOPER', 'VIEWER']);
export const deploymentStatusEnum = pgEnum('deployment_status', ['PENDING', 'BUILDING', 'RUNNING', 'STOPPED', 'FAILED']);
export const vpsStatusEnum = pgEnum('vps_status', ['CREATING', 'RUNNING', 'STOPPED', 'REBOOTING', 'TERMINATED']);
export const databaseStatusEnum = pgEnum('database_status', ['CREATING', 'AVAILABLE', 'BACKING_UP', 'MAINTENANCE', 'FAILED']);
export const billingStatusEnum = pgEnum('billing_status', ['ACTIVE', 'PAST_DUE', 'SUSPENDED', 'CANCELLED']);
export const subscriptionStatusEnum = pgEnum('subscription_status', ['ACTIVE', 'PAST_DUE', 'CANCELLED', 'UNPAID']);
// Organizations table
...
export const databaseInstancesRelations = relations(databaseInstances, ({ one }) => ({
    organization: one(organizations, {
